...
class IPCrossAttention(CrossAttention):
    def __init__(self, query_dim: int,
        ...
        self.to_k_ip = nn.Linear(image_cross_attention_dim or query_dim, query_dim, bias=False)
        self.to_v_ip = nn.Linear(image_cross_attention_dim or query_dim, query_dim, bias=False)
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, attention_mask=None):
        ...
        if isinstance(encoder_hidden_states, tuple):
            encoder_hidden_states, ip_hidden_states = encoder_hidden_states[0], encoder_hidden_states[1]
        else:
            end_pos = encoder_hidden_states.shape[1] - self.num_tokens
            encoder_hidden_states, ip_hidden_states = encoder_hidden_states[:, :end_pos, :], encoder_hidden_states[:, end_pos:, :]
            if self.image_cross_attention_dim != self.cross_attention_dim:
                ip_hidden_states = ip_hidden_states[:,:,:self.image_cross_attention_dim]
        ...
        if self.use_learnable_scale:
            print(f'{self.scale}-{self.alpha}-{torch.tanh(self.alpha)+1}-{self.scale*(torch.tanh(self.alpha)+1)}')
            hidden_states = hidden_states + self.scale * ip_hidden_states * (torch.tanh(self.alpha)+1)
        else:
...
class Transformer3DModel(ModelMixin, ConfigMixin):
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, timestep=None, return_dict: bool = True):
        ...
        if isinstance(encoder_hidden_states, tuple):
            encoder_hidden_states_text = repeat(encoder_hidden_states[0], 'b n c -> (b f) n c', f=video_length)
            encoder_hidden_states_ip = repeat(encoder_hidden_states[1], 'b n c -> (b f) n c', f=video_length)
            encoder_hidden_states = (encoder_hidden_states_text, encoder_hidden_states_ip)
        else:
...
class BasicTransformerBlock(nn.Module):
    def __init__(
        ...
        self.use_ip_plus_cross_attention = use_ip_plus_cross_attention
        ...
        if cross_attention_dim is not None:
            if use_ip_plus_cross_attention:
                self.attn2 = IPCrossAttention(
                    ...
                    bias=attention_bias,
                    upcast_attention=upcast_attention,
                    scale=scale, num_tokens=num_tokens, use_learnable_scale=use_learnable_scale
                )
    ...
    def forward(self, hidden_states, encoder_hidden_states=None, timestep=None, attention_mask=None, video_length=None, image_length=0):
